...
public class TestSequenceRead extends BookKeeperClusterTestCase {
    ...
    private LedgerHandle createLedgerWithDuplicatedBookies() throws Exception {
        ...
        SortedMap<Long, ArrayList<InetSocketAddress>> newEnsembles = new TreeMap<Long, ArrayList<InetSocketAddress>>();
        for (Map.Entry<Long, ArrayList<InetSocketAddress>> entry : ensembles.entrySet()) {
            ArrayList<InetSocketAddress> newList = new ArrayList<InetSocketAddress>(entry.getValue().size());
            InetSocketAddress firstBookie = entry.getValue().get(0);
            for (InetSocketAddress ignored : entry.getValue()) {
