...
public final class NativeIO {
    ...
    public static void bestEffortRemoveFromPageCache(FileDescriptor fileDescriptor, long offset, int len) {
        ...
        try {
            ...
            LOG.warn("Unsatisfied Link error: posix_fadvise failed on file descriptor {}, offset {}", fileDescriptor, offset);
        }
        catch (Exception e) {
            // This is best effort anyway so lets just log that there was an
            // exception and forget
            LOG.warn("Unknown exception: posix_fadvise failed on file descriptor {}, offset {}", fileDescriptor, offset);
        }
    }
